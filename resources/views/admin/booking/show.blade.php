...
@section('content')
    ...
    <div class="row my-3">
                ...
                <div class="card-body">
                    ...
                    <div class="d-flex justify-content-between flex-wrap gap-2">
                        <div class="d-flex flex-column gap-2">
                            <div class="fw-bold">
                                {{ __('Booking Id') }}: #{{ $order->prefix . $order->booking_code }}
                            </div>
                            <div>
                        ...
                        <div>
                            <a href="{{ route('shop.booking-download-invoice', $order->id) }}" target="_blank" class="btn btn-primary">
                    ...
                    <div class="table-responsive mt-3">
                        <table class="table">
                            ...
                            <tbody>
                                @foreach ($order->bookingItems as $key => $product)
                                    <tr>
                                        <td>{{ $key + 1 }}</td>
                                        <td>
                                            <div class="d-flex gap-1 align-items-center">
                                                <img src="{{ $product->service->thumbnail }}" alt="" width="40"
                                        ...
                                        <td>
                                            {{ showCurrency($product->service->discounted_price > 0 ? $product->service->discounted_price  : $product->service->price) }}
                                        </td>
                                        <td>
                                            {{ showCurrency($product->quantity * ($product->service->discounted_price > 0 ? $product->service->discounted_price : $product->service->price)) }}
                                        </td>
                                    </tr>
                                @endforeach
                            </tbody>
        ...
        <div class="col-lg-4">
            ...
            <div class="card">
                <div class="card-body">
                    <h5>{{ __('Booking Info') }}</h5>

                    <div class="mt-3">
                        <div>{{ __('Change Booking Status') }}</div>
                                ...
                                @foreach ($orderStatus as $satus)
                                    <li>
                                        <a class="dropdown-item"
                                            href="{{ route('admin.booking.status.change', $order->id) }}?status={{ $satus->value }}">
                                            {{ __($satus->value) }}
                                        </a>
                                    </li>
                                @endforeach
                    ...
                    <div class="border rounded d-flex justify-content-between p-2 mt-3">
                        ...
                        <div class="d-flex align-items-center gap-1">
                            ...
                            <label class="switch mb-0">
                                <a href="{{ route('admin.booking.payment.status.toggle', $order->id) }}">
                                    <input type="checkbox" {{ $order->payment_status == 'Paid' ? 'checked' : '' }}>
