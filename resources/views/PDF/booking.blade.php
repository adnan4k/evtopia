    ...
    <div class="contains">
        ...
        <div class="display-table w-full">
            <div>
                ...
                <p class="fw-500 pt-1">
                    {{ ($order->customer->addresses[0]->address_line ?? 'N/A') . ', ' . ($order->customer->addresses[0]->address_line2 ?? 'N/A') . ', ' . ($order->customer->addresses[0]->area ?? 'N/A') }}
                </p>
        ...
        <table class="invoice-details">
            ...
            <tr>
                ...
                <td>#{{ $order->prefix . $order->booking_code }}</td>
        ...
        <table class="items-table">
            ...
            <tbody>
                @foreach ($order->bookingItems as $product)
                    @php
                        $price = $product->service->discounted_price > 0 ? $product->service->discounted_price : $product->service->price;
                    @endphp
                    <tr>
                        <td>{{ $loop->iteration }}.</td>
                        <td>
                            <span style="text-transform: capitalize">{{ $product->service->name }}</span>
                            <p class="pt-1 text-gray product-des">
                                {{ Str::limit($product->service->short_description, 60, '...') }}
                            </p>
                        </td>
                        <td class="text-center fw-400">{{ showCurrency($price) }}</td>
                        <td class="text-center">{{ $product->quantity }}</td>
                        <td class="text-right">{{ showCurrency($price * $product->quantity) }}</td>
                    </tr>
                @endforeach
            </tbody>
        </table>
