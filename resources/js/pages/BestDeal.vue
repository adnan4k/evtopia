<template>
    <div class="main-container pt-8 pb-12">
        ...
        <div class="text-slate-800 text-lg lg:text-3xl font-bold">{{ $t('Special Offers') }}</div>

        <!-- Products -->
        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4  gap-6 items-start mt-6">
...
const fetchProducts = async () => {
    axios.get('/special-products', { params: { page: currentPage.value, per_page: perPage.value, sort_type: 'top_selling' } }).then((response) => {
        totalProducts.value = response.data.data.total;
        products.value = response.data.data.special_offers;
    })
};
