                ...
                <div class="max-w-[920px] mx-auto bg-white rounded-xl md:rounded-2xl p-3 md:p-4 xl:p-6">
                    ...
                    <div class="p-3 bg-white rounded-xl border border-slate-100 mt-3">
                        <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
                            ...
                            <div>
                                <label for="make" class="form-label">Make (Manufacturer)</label>
                                <input type="make" id="make" v-model="formData.make" class="form-input"
                                    placeholder="Enter Manufacturer"
                                    :class="(errors && errors?.make) ? 'border-red-500' : 'border-slate-200'">
                                <span v-if="errors && errors?.make" class="text-red-500 text-sm">
                                    {{ errors?.make[0] }}
                                </span>
                            </div>
                            ...
                            <div>
                                ...
                                <input type="model" id="model" v-model="formData.model" class="form-input"
                                    placeholder="Enter Manufacturer"
                                    :class="(errors && errors?.model) ? 'border-red-500' : 'border-slate-200'">
                                <span v-if="errors && errors?.model" class="text-red-500 text-sm">
                                    {{ errors?.model[0] }}
                                </span>
                            </div>
                            ...
                            <div>
                                ...
                                <input type="year" id="year" v-model="formData.year" class="form-input"
                                    placeholder="Enter Year"
                                    :class="(errors && errors?.year) ? 'border-red-500' : 'border-slate-200'">
                                <span v-if="errors && errors?.year" class="text-red-500 text-sm">
                                    {{ errors?.year[0] }}
                                </span>
                            </div>
                            ...
                            <div>
                                ...
                                <input type="date" id="service_date" v-model="formData.service_date" class="form-input"
                                    placeholder="Enter Service Date"
                                    :class="(errors && errors?.service_date) ? 'border-red-500' : 'border-slate-200'">
                                <span v-if="errors && errors?.service_date" class="text-red-500 text-sm">
...
const formData = ref({
    ...
    make: authStore.user?.vehicle?.make,
    model:authStore.user?.vehicle?.model,
    year: authStore.user?.vehicle?.year,
    gender: authStore.user?.gender ? authStore.user?.gender : '',
    date_of_birth: authStore.user?.date_of_birth,
    email: authStore.user?.email,
    service_date:authStore.user?.vehicle?.service_date,
    profile_photo: null,
...
const sumitForm = () => {
    ...
    processData.append('make', formData.value.make ? formData.value.make : '');
    processData.append('model', formData.value.model ? formData.value.model : '');
    processData.append('year', formData.value.year ? formData.value.year : '');
    processData.append('service_date', formData.value.service_date ? formData.value.service_date : '');
