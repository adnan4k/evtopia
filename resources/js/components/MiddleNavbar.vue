    ...
    <div class="main-container py-2 flex items-center justify-between gap-8">
        ...
        <div class="flex items-center gap-8 grow">
            ...
            <div class="relative overflow-hidden grow max-w-[800px] hidden md:block">
                ...
                <button
                    class="bg-primary h-full w-14 border-none absolute right-0 top-0 rounded-r-lg flex items-center justify-center" @click="searchProducts()">
...
<!-- 
     ...
     <TransitionRoot as="template" :show="showSearchOverlay">
        <div        
         @click="handleClickOutside"
        class="fixed inset-0 z-20 bg-black bg-opacity-50 flex items-center justify-center p-4">
            <!-- Search Box Container -->
            <div @click.stop class="relative w-full max-w-md bg-white p-6 rounded-lg shadow-lg">
                <!-- Close Button -->
                <button @click="closeSearch" class="absolute top-[2px] border rounded-full border-red-500 text-red-500 right-[2px] ">
                    <XMarkIcon class="w-6 h-6" />
                </button>
    
                <!-- Search Input -->
                <input type="text" v-model="search" placeholder="Search products..."
                    class="w-full px-4 py-3 mb-4 border border-slate-200 rounded-lg text-base placeholder-gray-400 focus:outline-none focus:border-primary">
                
                <!-- Search Button -->
                <button @click="searchProducts" class="w-full py-3 bg-primary text-white rounded-lg">
                    Search
                </button>
            </div>
        </div>
...
const handleClickOutside = (event) => {
    // Close the overlay if the clicked element is outside the search box
    if (!event.target.closest('.relative')) {
        showSearchOverlay.value = false;
    }

    const basketModal = document.getElementById('basketCanvasCustom');
    if (basketModal) {
...
const showSearchInput = () => {
    ...
    const basketModal = document.getElementById('basketCanvasCustom');
    if (basketModal) {
        basketModal.style.display = 'none'; // Hide the basket modal
    }
