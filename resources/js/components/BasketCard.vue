        ...
        <div v-if="routerName !== 'checkout'"
            ...
            <div class="px-2 py-1 bg-primary">
                <div class="w-14 text-center text-white text-xs font-bold">
                    {{ master.showCurrency(ServiceStore.total_amount + parseFloat(BasketStore.totalAmount)) }}
                            ...
                            <TransitionChild as="template"
                                ...
                                <DialogPanel class="pointer-events-auto relative w-screen max-w-md">
                                    ...
                                    <div class="flex h-full flex-col overflow-y-scroll bg-white shadow-xl">
                                        <div class="p-6 border-b border-slate-200 flex justify-between">
                                            ...
                                            <DialogTitle class="flex gap-3 items-center">
                                                ...
                                                <div class="text-primary-600 text-lg font-medium  tracking-tight">
                                                    ({{BasketStore.total + ServiceStore.total }} {{ $t('items') }})
                                                </div>
                                            </DialogTitle>
                                        ...
                                        <!-- Cart Items -->
                                        <div v-if="BasketStore.total > 0" class="relative  py-3 px-4"
                                            :class="ServiceStore.total<=0 ? 'flex-1' : ''"
                                            >
                                            <h1 class="font-bold flex justify-center pb-1">{{ 'Product Cart Items' }}</h1>
                                            <Basket />
                                        </div>


                                        <div v-if="ServiceStore.total > 0" class="relative flex-1 py-3 px-4">
                                            <h1 class="font-bold flex justify-center pb-1">{{ 'Service Cart Items' }}</h1>

                                            <ServiceBasket />
                                        </div>

                                        <div v-if="BasketStore.total <= 0 && ServiceStore.total <= 0" class="flex flex-1 items-center justify-center flex-col gap-3">
                                            <div
                                        ...
                                        <!-- Proceed to Checkout -->
                                        <div v-if="(BasketStore.total + ServiceStore.total_amount) > 0"
                                            ...
                                            <div>
                                                ...
                                                <div
                                                    ...
                                                    {{ master.showCurrency(BasketStore.total_amount + ServiceStore.total_amount) }}</div>
                                            </div>
                                            <button
...
import ServiceBasket from './ServiceBasket.vue';
...
const processToCheckout = () => {
    ...
    if ( BasketStore.total > 0 && BasketStore.selectedShopIds.length === 0) {
