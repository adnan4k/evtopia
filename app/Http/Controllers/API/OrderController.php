...
class OrderController extends Controller
{
    ...
    public function verify_payment( Request $request)
    {
        ...
        $gateway = new ChapaService();
        $response = $gateway->chapaCallback($request->reference);
    ...
    public function storeV1(OrderRequestV1 $request)
    {
        ...
        $service_carts = auth()->user()->customer->service_carts()->where('is_buy_now', $isBuyNow)->get();

        if ($carts->isEmpty() && $service_carts->isEmpty()) {
        ...
        // Store the order
        $payment = OrderRepository::storeByrequestFromCart($request, $paymentMethod, $carts, $service_carts);
        $paymentGateway = PaymentGateway::where('name', $request->payment_method)->first();

        $paymentUrl = null;
        if ($paymentMethod->name != 'CASH' && $paymentMethod->name != 'CHAPA') {
            $paymentUrl = route('order.payment', ['payment' => $payment, 'gateway' => $request->payment_method]);
        }
        $response = null;
    ...
    public function booking_details(Request $request)
    {
        ...
        return $this->json('order details', [
            'order' => BookingDetailsResource::make($order),
        ]);
    }
