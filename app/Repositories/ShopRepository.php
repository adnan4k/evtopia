...
class ShopRepository extends Repository
{
     ...
     */
    public static function storeByRequest(Request $request): Shop
    {
        // create new user
        ...
        //shop logo
        $thumbnail = MediaRepository::storeByRequest($request->shop_logo?$request->shop_logo:$request->profile_photo, 'shops/logo', 'image');

        //shop banner
        $banner = null;
        if ($request->hasFile('shop_banner')) {
            $banner = MediaRepository::storeByRequest($request->shop_banner?$request->shop_banner:$request->profile_photo, 'shops/banner', 'image');
        }

        // create new shop and return
        return self::create([
            'user_id' => $user->id,
            'name' => $request->shop_name?$request->shop_name:$request->first_name,
            'logo_id' => $thumbnail ? $thumbnail->id : null,
            'banner_id' => $banner ? $banner->id : null,
            'delivery_charge' => $request->delivery_charge ?? 0,
            'address' => $request->address??'AA',
    ...
    public static function updateByRequest(Shop $shop, $request): Shop
    {
        ...
        // update shop
        self::update($shop, [
            'name' => $request->shop_name??$shop->name,
            'logo_id' => $thumbnail ? $thumbnail->id : null,
            'banner_id' => $banner ? $banner->id : null,
            'delivery_charge' => $request->delivery_charge ?? 0,
