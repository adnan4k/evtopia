...
class OrderRepository extends Repository
{
     ...
     */
    public static function storeByrequestFromCart(OrderRequestV1 $request, $paymentMethod, $carts, $service_carts)
    {
        ...
        if($service_carts->isNotEmpty()){
            $booking = self::createNewBooking($request,  $paymentMethod, $service_carts);
            $payment->bookings()->attach($booking->id);
            $payment->update([
                'amount' => $totalPayableAmount + $booking->total_amount,
            ]);
        }
        ...
        $isBuyNow = $request->is_buy_now ?? false;
         ...
         auth()->user()->customer->service_carts()->where('is_buy_now', $isBuyNow)->delete();
    ...
    private static function createNewBooking($request, $paymentMethod, $service_carts)
    {
        $lastBooking = Booking::query()->max('id');
        $checkout = ServiceCartRepository::checkoutByRequest($request, $service_carts);

        $booking = Booking::create([
            'booking_code' => str_pad($lastBooking + 1, 6, '0', STR_PAD_LEFT),
            'prefix' => $shop->prefix ?? 'SV',
            'customer_id' => auth()->user()->customer->id,
            'coupon_id' => null,
            'payable_amount' => $checkout['total_amount'],
        ...
        foreach ($service_carts as $cartItem) {
            BookingItem::create([
                ...
                'unit_price' => $cartItem->service->discounted_price > 0
                ? $cartItem->service->discounted_price
